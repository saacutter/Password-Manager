...
def edit_password(id):
    ...
    key = Fernet(base64.urlsafe_b64encode(current_user.password.encode('utf-8').ljust(32)[:32]))
    password_entry = controllers.decrypt_password(key, models.Passwords.query.get_or_404(id))
    ...
    if request.method == "POST" and form.validate_on_submit():
        ...
        # Update the password information based on the provided information
        password_entry = models.Passwords.query.get_or_404(id)
        password_entry.name = name
        password_entry.username = username
        password_entry.password = password
        password_entry.website = website
        password_entry.details = details
    ...
    return render_template("edit-password.html", form=form, password=password_entry)
...
@login_required
def delete_account():
    # Delete the passwords associated with the user
    controllers.purge_passwords(current_user, db)

    # Delete the passwords and commit the changes to the database
    db.session.delete(models.Users.query.get(current_user.id))
    db.session.commit()
    return redirect(url_for('index'))
